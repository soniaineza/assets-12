...
export function AssetDetail() {
  ...
  {
    label: 'Serial Number',
    value: fmt(asset.serialNumber)
  },
  {
    label: 'Acquisition Date',
    value: new Date(asset.acquisitionDate).toLocaleDateString('en-GB', {
      day: '2-digit',
      month: 'long',
      year: 'numeric'
    })
  ...
  {
    label: 'Value',
    value: `RWF ${asset.value.toLocaleString(undefined, {
  ...
  {
    label: 'Assigned To',
    value: fmt(asset.assignedTo)
  },
  {
    label: 'Supplier',
    value: fmt(asset.supplier)
  },
  ...(asset.insuranceExpiry ?
  [
  {
    label: 'Insurance Expiry',
    value: new Date(asset.insuranceExpiry).toLocaleDateString('en-GB', {
      day: '2-digit',
      month: 'long',
      year: 'numeric'
    })
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-paper-light border-2 border-ink">
        ...
        {/* Detail grid - ledger style */}
        <dl className="grid grid-cols-1 sm:grid-cols-2">
          {fields.map((f, i) =>
          <div
            key={f.label}
            className={`px-6 py-4 border-rule ${i < fields.length - 1 ? 'border-b' : ''} ${i % 2 === 0 ? 'sm:border-r' : ''} ${i === fields.length - 1 && fields.length % 2 !== 0 ? 'sm:col-span-2' : ''}`}>
            
              <dt className="text-[11px] uppercase tracking-wider text-ink-muted font-semibold mb-1">
                {f.label}
              </dt>
              <dd className={`text-sm text-ink ${f.mono ? 'font-mono' : ''}`}>
                {f.value}
              </dd>
            </div>
          )}
      ...
      <div className="bg-paper-light border border-rule">
        <div className="px-6 py-3 border-b border-rule bg-paper-dark/40">
          <h3 className="font-serif text-lg text-ink">Record History</h3>
        </div>
        <div className="p-6">
          {logs.length > 0 ?
